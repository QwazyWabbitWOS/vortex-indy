...
void PlayerBossPoints (edict_t *attacker, edict_t *target)
{
	...
	//float		levelmod, dmgmod=0;
	char		*message;
	//edict_t		*cl_ent;
	//qboolean	killteamboss=false;
	//dmglist_t	*slot=NULL;

	attacker = G_GetClient(attacker);
	if (!attacker) // don't award boss points if a player doesn't kill him
	{
		gi.dprintf("Boss killed by world.\n");
		return;
	}
	...
	//if (pvm->value)
	//{
		AwardBossKill(target);
		//return;
	//}

	//printDmgList(target); // for testing
/*
	if (findNearbyBoss(target))
	{
		message = HiPrint(va("Players were awarded extra points for killing a boss team!"));
		gi.bprintf(PRINT_HIGH, "%s\n", message);
		killteamboss = true;
	}

	// find the player that did the most damage
	slot = findHighestDmgPlayer(target);
	...
	for (i=0 ; i<game.maxclients ; i++) 
	{
		...
		// calculate base points and credits based on level
		points = BOSS_EXPERIENCE*levelmod;
		credits = BOSS_CREDITS*levelmod;
		// extra points for killing a boss team
		if (killteamboss)
			points *= BOSS_ALLY_BONUSMOD;
		if (slot && (cl_ent == slot->player))
		{
			dmgmod = 100*(slot->damage/GetTotalBossDamage(target));
			message = HiPrint(va("%s got a hi-damage bonus! %d damage (%.1f%c)", 
				slot->player->client->pers.netname, (int)slot->damage, dmgmod, '%'));
			gi.bprintf(PRINT_HIGH, "%s\n", message);
			points *= BOSS_DAMAGE_BONUSMOD;
		}
