...
void Think_AccelMove (edict_t *ent)
{
//	gi.dprintf("%s origin %d end_origin %d remaining_distance %d\n", ent->classname,
//		(int)ent->s.origin[2], (int)ent->moveinfo.end_origin[2], (int)ent->moveinfo.remaining_distance);
...
void SP_func_door (edict_t *ent)
{
	...
	ent->solid = SOLID_BSP;
	gi.setmodel (ent, ent->model);
	ent->mtype = FUNC_DOOR;//GHz - so AI can identify this as a door
